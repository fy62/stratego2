class Piece {
  constructor(type, player, x = null, y = null) {
    this.type = type;
    this.player = player;
    this.x = x;
    this.y = y;
...
class Board {
  constructor(redOrderedPieces, blueOrderedPieces, currentPlayer = 'red') {
    this.board = createBoard();
    if (redOrderedPieces[0] instanceof Piece) {
      this.redPieces = redOrderedPieces;
      this.bluePieces = blueOrderedPieces;
      this.placePieces();
    }
    else if (!redOrderedPieces[0].id) {
      this.redPieces = createRedPieces(redOrderedPieces, this);
      this.bluePieces = createBluePieces(blueOrderedPieces, this);
    }
    else {
      this.redPieces = redOrderedPieces.map(piece => new Piece(piece.type, piece.color, piece.x, piece.y));
      this.bluePieces = blueOrderedPieces.map(piece => new Piece(piece.type, piece.color, piece.x, piece.y));
      this.placePieces();
    }
  ...
  placePieces() {
    this.redPieces.forEach((piece) => {
      this.board[piece.y][piece.x].piece = piece;
    });
